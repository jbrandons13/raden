...
## F. Sistem FROZEN 🆕❄️ (proyek besar — fase tersendiri)
...
**Alur:**
- ☐ Migration: role `admin_frozen` + semua tabel `frozen_*` + RLS
- ☐ **進貨** (barang masuk): produk + qty + EXP → tambah batch + log movement (+)
- ☐ **Stok**: tampilan **Total** + **Detail per-EXP** (urut EXP terdekat)
- ☐ **出貨**: input manual / **upload Excel** → draft (stok belum dipotong)
- ☐ **確認 (lock)**: alokasi **FEFO** via **RPC atomik** → potong batch + log (−) + 撿貨單 + invoice customer
- ☐ **Revisi**: unlock → balikin stok lama → edit → 確認 ulang → potong ulang FEFO
- ☐ **Back Order**: stok kurang → simpan, **tidak bisa 確認** sampai stok cukup
...
## ⚖️ Saran urutan (sesuai budget jatah)
- **Fase 1 (quick win):** (6) → (1)+(4)
- **Fase 2:** (2) → (3)+(5) → (7)
- **Fase 3 (besar):** FROZEN
